...
// --- COMPONENTE: FONDO DINÁMICO DE PARTÍCULAS MEJORADO ---
...
const Particle = ({ index }: { index: number }) => {
  ...
  const yPos = useRef(Math.random() * SCREEN_HEIGHT).current;
  const size = useRef(15 + Math.random() * 60).current; // Más grandes
  const duration = useRef(8000 + Math.random() * 12000).current; // Más rápidas
  ...
  const translateY = anim.interpolate({
    inputRange: [0, 1],
    outputRange: [SCREEN_HEIGHT * 0.2, -SCREEN_HEIGHT * 0.4], // Movimiento más amplio
  });

  const translateX = anim.interpolate({
    inputRange: [0, 0.5, 1],
    outputRange: [0, 30, -30], // Movimiento lateral
  });

  const opacity = anim.interpolate({
    inputRange: [0, 0.3, 0.7, 1],
    outputRange: [0, 0.4, 0.4, 0], // Mucho más visibles (0.4 vs 0.15)
  });
          ...
          <View style={[styles.infoCard, { padding: select({ mobile: 24, tablet: 40, desktop: 60 }) }]}>
            ...
            <View style={styles.section}>
              ...
              <Text style={styles.bodyText}>
                {description ? renderBoldBrand(description) : "Información en proceso..."}
              </Text>
            </View>
          ...
          <View style={styles.modalOverlay}>
            ...
            <Animated.View style={styles.modalContent}>
              ...
              <Text style={styles.modalMessage}>
                Para agendar con nuestros especialistas, primero crea tu cuenta en {renderBoldBrand('VIREM')}.
              </Text>
