...
// --- 1. VALIDACIÓN DE FECHA REAL ---
const esFechaValida = (fechaStr: string) => {
    if (fechaStr.length !== 10) return false;

    const [dia, mes, anio] = fechaStr.split('/').map(Number);
    
    // Crear objeto fecha (el mes en JS empieza en 0, por eso mes - 1)
    const fecha = new Date(anio, mes - 1, dia);

    // Verificar si la fecha es lógica (Ej: que no sea 30 de febrero)
    const esLogica = fecha.getFullYear() === anio && fecha.getMonth() === mes - 1 && fecha.getDate() === dia;
    ...
    // Verificar que no sea una fecha demasiado antigua (ej: más de 120 años)
    if (anio < (hoy.getFullYear() - 120)) return false;

    return true;
};

// --- 2. VALIDACIÓN DE CÉDULA DOMINICANA ---
...
const styles = StyleSheet.create({
    ...
    helpLink: { color: colors.blueGray, fontSize: 14, fontWeight: 'bold', flexDirection: 'row', alignItems: 'center', gap: 4 },
    ...
    pageSubtitle: { color: colors.blueGray, fontSize: 16, textAlign: 'center', maxWidth: 600, marginHorizontal: 'auto' },
    ...
    inputWrapper: { flex: 1 },
    selectInput: { height: 48, borderRadius: 8, borderWidth: 1, borderColor: colors.navyMedium, backgroundColor: colors.slate50, paddingHorizontal: 16, justifyContent: 'center', position: 'relative' },
    ...
    continueButton: { width: width > 640 ? 'auto' : '100%', height: 48, paddingHorizontal: 32, borderRadius: 8, backgroundColor: colors.primary, fontWeight: 'bold', shadowColor: colors.primary, shadowOffset: { width: 0, height: 4 }, shadowOpacity: 0.2, shadowRadius: 5, elevation: 5, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 8 },
...
const RegistroPacienteScreen: React.FC = () => {
    ...
    const [fechaError, setFechaError] = useState(false); // Estado para error de fecha

    const isFormComplete = names.trim() !== '' && lastNames.trim() !== '' && birthDate.trim() !== '' && gender !== '' && cedula.trim() !== '' && phone.trim() !== '';
    ...
    const handleContinue = async () => {
        ...
        // --- AQUÍ EL ÚNICO CAMBIO: SE ENVÍAN LOS DATOS ---
    ...
    const completedFields = [names, lastNames, birthDate, gender, cedula, phone].filter(field => field.trim() !== '').length;
                            ...
                            <View style={styles.formRow}>
                                <View style={styles.inputWrapper}>
                                    ...
                                    <TextInput 
                                        ...
                                        placeholder="402XXXXXXX9"
                                        keyboardType="numeric"
                                        value={cedula}
                                        onChangeText={(t) => { setCedula(t.replace(/\D/g, '')); setCedulaError(false); }}
                                        maxLength={11}
                                    />
                                ...
                                <View style={styles.inputWrapper}>
                                    ...
                                    >
                                        <Text style={{ color: gender ? colors.navyDark : colors.blueGray }}>{gender || 'Seleccionar'}</Text>
                                    </TouchableOpacity>
                                </View>
                            </View>
                                ...
                                <View style={styles.inputWrapper}>
                                    ...
                                    <View style={[styles.phoneInputGroup, showErrors && !phone && styles.inputError]}>
                                        ...
                                        <TextInput 
                                            ...
                                            onChangeText={(text) => setPhone(applyPhoneMask(text, selectedCountryCode.mask))}
                                        />
                                    </View>
                                </View>
                                <View style={styles.inputWrapper}>
                                    ...
                                    <TextInput 
                                        style={[styles.inputField, (showErrors && !birthDate || fechaError) && styles.inputError]}
                                        placeholder="DD/MM/YYYY"
                                        value={birthDate}
                                        onChangeText={(t) => { formatAndSetDate(t, setBirthDate); setFechaError(false); }}
                                        keyboardType="numeric"
                                        maxLength={10}
                                    />
                    ...
                    <View style={styles.modalContent}>
                        <ScrollView>
                            {countryCodes.map((c, i) => (
                                <TouchableOpacity key={i} style={styles.modalOption} onPress={() => {setSelectedCountryCode(c); setPhone(''); setShowPrefixModal(false)}}>
