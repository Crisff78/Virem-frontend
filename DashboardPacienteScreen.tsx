...
const DashboardPacienteScreen: React.FC = () => {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      if (upcoming?.success) {
        setUpcomingCitas([...(upcoming.citas || [])].sort((a, b) => parseDateMs(a?.fechaHoraInicio) - parseDateMs(b?.fechaHoraInicio)));
      }
    } catch (error: any) {
...
const styles = StyleSheet.create({
  ...
  expressTitle: { color: '#fff', fontWeight: '900' },
  expressSubtitle: { marginTop: 2, color: '#94A3B8' },
  expressBtn: {
    backgroundColor: colors.primary,
    borderRadius: radii.md,
  ...
  expressBtnText: { color: '#fff', fontWeight: '800' },
});
